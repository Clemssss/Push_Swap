...
/*   Updated: 2018/01/19 18:28:59 by clegirar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		rec_b(t_info *info, int size, int recup_b, int count)
{
	...
	if (size <= 3)
	{
		...
		info->come = info->come == 0 ? 1 : 0;
		return (0);
	}
	info->mediane = fill_mediane(info->lb, size);
	push_in_a(info, &count, recup_b);
	info->come = 1;
	rec_b(info, size - count, recup_b, 0);
	info->come = 0;
	rec_a(info, count, 1, 0);
...
int		algo_push_swap(t_info *info)
{
	...
	rec_a(info, size_lst(info->la), 0, 0);
	op_inutile(info);
